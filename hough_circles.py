...
def namingConvention(test_no):
	with open('data.json') as data_file:
		data = json.load(data_file)
	return data["test"][test_no-1]
...
def crop(cimg,points,circles):
	...
	for i in range(0,len(points)-1):
		...
		if y2-y1 > points[i][2]+50 :
			...
			cv2.imwrite("/home/neel/opencvproject/REAP/answers/"+test_name+sub_code+str(part1)+part2+".jpg",roi)
		else:
...
if __name__ == '__main__':	
	...
	sub_code = input("\nEnter the subject code(Eg:12CS306) : ")
	#read the image
	...
	test_name = namingConvention(test_no)
